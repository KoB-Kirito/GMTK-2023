...
func on_character_joined(_info: Dictionary):
	...
	#tween.tween_property(%TextMargin, "theme_override_constants/margin_left", portrait_margin, animation_duration)
	tween.tween_method(tween_margin, portrait_default_margin, portrait_margin, animation_duration)
	
	# slide name in
	tween.parallel().tween_property(%NameContainer, "position:x", -8, animation_duration).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_BACK)
...
func on_character_left(_info: Dictionary):
	#print_debug("character left")
	
	#if %TextMargin.theme_override_constants.margin_left == portrait_default_margin:
	#	return
	
	var tween = create_tween()
	
	# move text to the left
	#tween.tween_property(%TextMargin, "theme_override_constants/margin_left", portrait_default_margin, animation_duration)
	tween.tween_method(tween_margin, portrait_margin, portrait_default_margin, animation_duration)

	# slide name in
	tween.parallel().tween_property(%NameContainer, "position:x", -235, animation_duration).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_BACK)
